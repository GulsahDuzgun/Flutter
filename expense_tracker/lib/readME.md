...
Flutter creates three treees. These are Widget Tree, Element Tree and Render Tree. The widget tree is simply combination of widgets. Flutter takes this widget tree and translate it to Element Tree which is simply in memory representation of your widgets. Widget Tree defines user interface & widget relations, build() is called frequesntly. Element Tree is created behind the scenes, elements are reused. Element Tree uses to determine necessary uı updates. Element objects are related to your widget objects. When a widget tree is encountered and its build method is caleld for the first time flutter will thereafter create a fitting element object and store that in memory. Build method typcally called quite frequently. The Element Tree created initially when build method is first called but for the subsequence cause of a widget's build method elements are reused. Flutter only created new element when onlt creates or remove a new widget. Elements are reused not created all the time. Render Tree is combination of visible ui building blocks. creating and updating that render tree is a lot of work so the build method only re-render parts of the ui when ui updates are needed. if an update needed it is determined with help of the element tree. Flutter partially re-rendered is ui updates are needed.
